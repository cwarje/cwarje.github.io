...
/** Presentation-only: icon and gradient per game. Copy comes from GAME_CATALOG. */
const GAME_UI: Record<GameType, { icon: typeof Dice5; gradient: string }> = {
  yahtzee: { icon: Dice5, gradient: 'from-amber-500/20 to-orange-600/20' },
  hearts: { icon: Heart, gradient: 'from-rose-500/20 to-pink-600/20' },
  battleship: { icon: Ship, gradient: 'from-cyan-500/20 to-blue-600/20' },
  'liars-dice': { icon: Crosshair, gradient: 'from-emerald-500/20 to-green-600/20' },
  poker: { icon: Club, gradient: 'from-violet-500/20 to-purple-600/20' },
  'up-and-down-the-river': { icon: ArrowUpDown, gradient: 'from-teal-500/20 to-sky-600/20' },
};
  ...
  return (
    ...
    >
      {onInfo && (
        <button
          type="button"
          aria-label={`About ${catalog.title}`}
          onClick={(e) => {
            e.stopPropagation();
            onInfo(gameType);
          }}
      ...
      <div className={`w-14 h-14 rounded-xl bg-gradient-to-br ${ui.gradient} flex items-center justify-center mb-4`}>
        <Icon className={`w-7 h-7 ${ICON_COLORS[gameType]}`} />
      </div>
      <h3 className="text-xl font-bold text-white mb-2">{catalog.title}</h3>
      <p className="text-sm text-gray-400 mb-4 leading-relaxed">{catalog.shortDescription}</p>
      <div className="flex items-center justify-between">
        <span className="text-xs font-medium text-gray-500 uppercase tracking-wider">{catalog.playersLabel}</span>
        {!disabled && (
